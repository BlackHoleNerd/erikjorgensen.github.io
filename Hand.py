...
def load_images(image_paths):
    return {wl: pygame.image.load(path).convert_alpha() for wl, path in image_paths.items()}


# Interpolate brightness between two images
def interpolate_images(img1, img2, alpha):
    arr1 = pygame.surfarray.pixels3d(img1).astype(np.float32)
    arr2 = pygame.surfarray.pixels3d(img2).astype(np.float32)
    interpolated = arr1 * (1 - alpha) + arr2 * alpha
    surface = pygame.surfarray.make_surface(interpolated.astype(np.uint8))
    return surface

def apply_rgb_filter(surface, rgb_color):
    # Normalize RGB color to [0, 1] and reshape for broadcasting
    color_array = np.array(rgb_color, dtype=np.float32).reshape((1, 1, 3)) / 255.0  # shape (1, 1, 3)

    # Convert surface to array (shape: height x width x 3)
    pixels = pygame.surfarray.array3d(surface).astype(np.float32)

    # Transpose to (height, width, 3) if needed (check your array layout)
    pixels = np.transpose(pixels, (1, 0, 2))  # Pygame loads as (width, height, 3)

    # Apply filter
    filtered = pixels * color_array  # Broadcasts correctly

    # Convert back to uint8 and transpose for pygame surface
    filtered = np.clip(filtered, 0, 255).astype(np.uint8)
    filtered = np.transpose(filtered, (1, 0, 2))  # back to (width, height, 3)

    return pygame.surfarray.make_surface(filtered)


pygame.display.set_mode((373,799))

# Wavelengths and image paths
image_paths = {

    10e-9: "C:/Users/jorge/PycharmProjects/CosmosSimulator/colors/1.jpg",
    100e-9: "C:/Users/jorge/PycharmProjects/CosmosSimulator/colors/2.jpg",
    1e-6: "C:/Users/jorge/PycharmProjects/CosmosSimulator/colors/3.jpg",
    10e-6: "C:/Users/jorge/PycharmProjects/CosmosSimulator/colors/4.jpg",
    100e-6: "C:/Users/jorge/PycharmProjects/CosmosSimulator/colors/5.jpg"
}

# Load the images
images = load_images(image_paths)

# Compute log10 of the available wavelengths
log_wavelengths = {math.log10(wl): wl for wl in image_paths.keys()}
sorted_logs = sorted(log_wavelengths.keys())


# Interpolate image at target wavelength
def get_interpolated_image(wavelength_m):
    log_target = math.log10(wavelength_m)

    for i in range(len(sorted_logs) - 1):
        low_log = sorted_logs[i]
        high_log = sorted_logs[i + 1]
        if low_log <= log_target <= high_log:
            wl_low = log_wavelengths[low_log]
            wl_high = log_wavelengths[high_log]
            alpha = (log_target - low_log) / (high_log - low_log)
            img = interpolate_images(gamma_correct0(images[wl_low]), gamma_correct0(images[wl_high]), alpha)
            return gamma_correct(img)

    # If out of bounds, return nearest
    if log_target < sorted_logs[0]:
        return (images[log_wavelengths[sorted_logs[0]]])
    else:
        return (images[log_wavelengths[sorted_logs[-1]]])


# Example usage
screen = pygame.display.set_mode((373,799))
...
while running:
    ...
    if keys[pygame.K_UP]:
        ...
        N0 = N0 / 1.1
    N = np.log(wavelength0) * N0
    pygame.display.flip()
    if keys[pygame.K_RIGHT]:
        N = N + 1
    elif keys[pygame.K_LEFT]:
        N = N - 1
    #frequency = 440*2**(-N/1000)#*(1/(4.35456*10**17))
    ...
    interpolated_image = get_interpolated_image(wavelength1)
    ...
    filtered_surface = apply_rgb_filter(interpolated_image,COLOR)
    grayscale_surface = apply_rgb_filter(interpolated_image, WHITE)
    #interpolated_image


    if type0 ==0:
        screen.blit(filtered_surface, (640-373, 1))
    if type0 ==1:
        screen.blit(grayscale_surface, (640-373, 1))
    if type0 ==2:
        screen.blit(interpolated_image, (640-373, 1))
    ...
    text_surface = font.render(f"Wavelength in nanometers: {round(wavelength0*5)}", True, (255, 255, 255))  # White text
